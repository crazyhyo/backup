...
<body>
  ...
  <a class="header-link" href="/CodePeople/MainHR.do">인재</a>
        ...
        <a href="/CodePeople/rehearsal/home.html"><button id="header-login-btn"
            class="btn btn-primary btn-sm">로그아웃</button></a>
      </div>
    </div>
  </div>
  <!-- 바디 -->
    ...
    <div id="body-container">
      ...
      <div class="nav-left-bar nav-my">
        <div class="nav-sticky">
          <ul class="list-group">
            <li class="list-group-item "><a class="nav-link" href="/CodePeople/MainHR.do">모든 인재보기</a></li>
                    ...
                    <div class="card card-cv">
                        ...
                        <div class="card-body card-right">
                           ...
                           <label style="display: inline-block; width: 80px; font-weight: bold;">직군  </label><span class="jgCodeNm" name="jgCodeNo" value="<%=myInfoVO.getJgCodeNo()%>"><%=myInfoVO.getJgCodeNm() %></span>
                            ...
                            <div>
                            <label style="display: inline-block; width: 80px; font-weight: bold;">자기소개 </label><span class="hrIntro"><br><%=myInfoVO.getHrIntro().replaceAll("\r\n", "<br>") %><</span>
                            </div>
...
<%		
			...
			for(int i = 0; i< eduList.size(); i++){
				...
				if(!edu.getEduCodeNm().equals("학력무관") ){
				 ...
				 if(eduSchNm.equals("0")){
					 eduSchNm = "기재사항 없음";
				 }
				 if(eduMajor.equals("0")){
					 eduMajor = "기재사항 없음";
				 }
...
<% 	
		...
		for(int i = 0; i < crrList.size(); i++){
			...
			%>
			    <div class="card">
                <div class="card-header">
                  <a class="btn title-btn" data-bs-toggle="collapse" href="#c<%=i%>">
                    <span class="board-title crrCompNm"  ><%=crr.getCrrCompNm() %></span>&nbsp;&nbsp;&nbsp;<span class="board-title jtCodeNo" ><%=crr.getJtCodeNm() %></span>&nbsp;&nbsp;(<span class="board-title crrYears" ><%=crr.getCrrYears() %></span>년차)
                  </a>
                </div>
                <!-- 게시글을 담고있는 곳 a 태그가 가르키는 곳 -->
                <div id="c<%=i%>" class="collapse " data-bs-parent="#accordion1">
                  <div class="card-body">
