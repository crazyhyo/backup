	...
	$.ajax({
		...
		success : function(res) {
			if(isNaN($(target).find('span').text())){
				...
				$(`#board-like-${like}`).text(1);
			}else{
				$(target).find('span').text(parseInt($(target).find('span').text()) + 1);
				$(`#board-like-${like}`).text(parseInt($(`#board-like-${like}`).text()) + 1);
			}
		},	
		error : function(xhr) {
			alert("상태 : " + xhr)
        		...
        		<div class="card">
	        		<div class="card-header">
		        		...
		        		<div class="board-icon-box">
			        		...
			        		<i class="far fa-thumbs-up"></i>&nbsp;<span class="board-like board-sm-font" id="board-like-${v.boardLike}">${v.boardLike}</span>&nbsp;&nbsp;
			        		<i class="far fa-comment"></i>&nbsp;<span class="board-comment board-sm-font" id="board-reply-${v.boardRepl}">${v.boardRepl}</span>&nbsp;&nbsp;
		        		</div>
	        		</div>
