...
<title><%-- <decorator:title default="Kim's System" /> --%>Kim's System</title>
		...
		<nav class="main-header navbar navbar-expand navbar-white navbar-light">
			...
			<ul class="navbar-nav">
				...
				<c:forEach items="${menuList }" var="menu">
					<li class="nav-item d-none d-sm-inline-block"><a href="javascript:goPage('<%=request.getContextPath() %>${menu.murl }','${menu.mcode }');subMenu_go('${menu.mcode }');" class="nav-link">${menu.mname }</a></li>
				</c:forEach>
			</ul>
				...
				<c:if test="${!empty loginUser}">
					<div class="user-panel mt-3 pb-3 mb-3 d-flex">
						...
						<div class="info">
							...
							<a href="tel:${loginUser.phone }">tel : ${loginUser.phone }</a> <br /> <a href="mailto:${loginUser.email }">email : ${loginUser.email }</a>
						</div>
					</div>
				</c:if>
