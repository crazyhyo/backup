		          	...
		          	<%-- 	<c:if test="${loginUser.id eq member.id }"> --%>
			          		...
			          		<div class="col-sm-3 text-center">
			          			<c:if test="${member.enabled eq 1}">
				          			<button type="button" onclick="stop_go();" id="stopBtn" class="btn btn-info" >비활성</button>
			          			</c:if>
			          			<c:if test="${member.enabled eq 0}">
			          				<button type="button" onclick="active_go();" id="activeBtn" class="btn btn-info" >활&nbsp;&nbsp;성</button>
			          			</c:if>
			          		</div>
		          		<%-- </c:if> --%>
