...
/*
 * 쿠키 정보를 다루기 위한 예제(웹서버와 브라우저는 애플리케이션이 사용되는 동안 필요한 값을
 * 쿠키를 통해 공유하여 상태를 유지함.)
 *
 *  1. 구성요소
 *  - 이름
 ...
 *    => 쿠키의 도메인이 쿠키를 생성한 서버의 도메인을 벗어나면 브라우저는 쿠키를 저장(생성)
 *    하지 않는다.(보안때문)
 *  - 경로 : 쿠키를 공유할 기본 경로를 지정한다.(도메인 이후부분으로 디렉토리 수준)
 *    => 지정하지 않으면 실행한 URL의 경로부분이 사용됨.
 *  2. 동작방식
 *  - 쿠키생성단계 : 생성한 쿠키를 응답데이터의 헤더에 지정하여 웹브라우저에 전송한다.
 ...
 */
     ...
     * => 이외의 문자(예를 들면 한글)을 사용시에는 URLEncoder.encode() 사용하여 
     *    인코딩 처리를 해주면 된다.
     * 
     * 2. 쿠키 최대 지속시간을 설정한다.(초단위) => 지정하지 않으면 웹브라우저를 종료할 때 
     *    쿠키를 함께 삭제한다.
     *    cookie.setMaxAge(60*60*24); // 24시간
     *
     * 3. 응답헤더에 쿠키 객체를 추가한다.
     *    response.addCookie(cookie);
     *    // 톰캣이 서버의 응답을 클라이언트(브라우저)에 보낼 때 addCookie 명령을
     *    // setCookie태그를 추가하는 것으로 처리함. 브라우저는 이 명령으로 쿠키를 생성함.
     ...
     */
        ...
        // 쿠키 소멸시간 설정(초단위) => 지정하지 않으면 웹브라우저를 종료할 때 함께 삭제함.
        userId.setMaxAge(60*60*24); // 1일
        // javascript를 이용한 직접 접근방지(XSS 공격방지)
