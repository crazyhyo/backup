...
public class HotelManage {
				...
				ObjectInputStream ois = 
						new ObjectInputStream(
								new BufferedInputStream(
										new FileInputStream("d:/D_Other/room.bin")));				
				) {
			Object obj = null;
			while((obj = ois.readObject()) != null) {
				Room tmpRoom = (Room) obj;
				roomMap.put(tmpRoom.getRoomNo(), tmpRoom);
			}
		}catch(IOException e) {
				...
				ObjectOutputStream oos = 
						new ObjectOutputStream(
								new BufferedOutputStream(
										new FileOutputStream("d:/D_Other/room.bin")));
				){
	...
	private void showRoom() {
		...
		System.out.println();
		System.out.println(roomMap.get(0));
	}
	...
	private void checkIn() {
		...
		System.out.println(roomMap.get("101"));
