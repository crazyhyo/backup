...
public class Account {
	...
	void getAvg(int i){
		...
		int cmpQtt = 0;
		double tempSum = 0;
		String[] coinNames = {"비트코인","리플","도지코인"};
		for (TradeRecord walk : tradeRecord) {
			if (walk.coinInfo.name.equals(coinNames[i])) {
		...
		for(TradeRecord walk : tradeRecord){
			if(tradeRecord.indexOf(walk) >= index && walk.coinInfo.name.equals(coinNames[i])){
